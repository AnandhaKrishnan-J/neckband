...
# Load the YOLO model
model = YOLO('E:/Mini Project/Neckband/neck5_v11.pt')  # Replace with the path to your trained model
...
# Dictionary to store confidence values per class
...
while True:
    ...
    if results:
            ...
            for box in results[0].boxes:
                ...
                #A dictionary of detected class ids as key and list of confidences of that id
                ...
                #Once 10 instaces of currency detected , the following code is executed
                if len(selector) == 10:
                    ...
                    for class_id,conf_list in class_confidences.items():
                        avg_confidence[class_id] = sum(conf_list)/len(conf_list)
                    
                    if avg_confidence:
                        most_common_class_id = max(avg_confidence, key = avg_confidence.get)
                        max_avg_conf = avg_confidence[most_common_class_id]

                        print(avg_confidence)

                        class_name = model.names[most_common_class_id]
                        print(f"Most frequent class: {class_name} (ID: {most_common_class_id}), max avg conf: {max_avg_conf}")
